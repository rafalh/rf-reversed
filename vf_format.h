...
{
    int32_t     signature;        // should be equal to VF_SIGNATURE
    int32_t     version;          // font version (0 or 1)
    uint32_t    format;           // exists if version >= 1, else font has VF_FMT_MONO_4 format, for values
                                  // description see vf_format_t
    int32_t     num_chars;        // length of vf_file::chars array
    int32_t     first_ascii;      // ascii code of first character supported by this font, usually equals
                                  // 0x20 (space character)
    int32_t     default_spacing;  // spacing used for characters missing in the font (lower than first_ascii and
                                  // greater than first_ascii + num_chars)
    int32_t     height;           // font height (height is the same for all characters)
    int32_t     num_kern_pairs;   // length of vf_file::kern_pairs array
...
{
    int32_t     spacing;              // base spacing for this character (can be modified by kerning data), spacing is
                                      // similar to width but is used only during text rendering to update X coordinate
    int32_t     width;                // character width in pixels, not to be confused with spacing
    ...
    int16_t     first_kerning_entry;  // index in vf_file::kern_pairs array, entries can be checked from there because
